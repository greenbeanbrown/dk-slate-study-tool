...
def contest_summary_content(tab, data):
    ...
    if (data is None):
        ...
        df = summarize_mlb_lineup_stacks(df, points_ownership_df, PLAYER_TEAM_POS_DF)
        ...
        return [
            ...
            convert_mlb_df_to_html(stack_frequency_df, style=None, page_size=10)]),
...
def aggregate_exposures_tab_content(tab, data, agg_exposures_dropdown_selection):
    ...
    # If there is data then start processing the relevant tab data
        ...
        if (agg_exposures_dropdown_selection is None):
            df = prep_raw_mlb_contest_data(df, 'MLB')[0][['player','nickname','position','points','ownership']]
        # If there are users in the dropdown selection, then filter based on those users
            ...
            df = filter_dk_users(prep_raw_mlb_contest_data(df, 'MLB')[1], prep_raw_mlb_contest_data(df, 'MLB')[0], dk_users)
        ...
        return html.Div([
            ...
            convert_mlb_df_to_html(df, style='conditional')
        ])
...
def individual_lineups_tab_content(tab, data, dropdown_selection):
    ...
    if (data is None) or (dropdown_selection is None):
        ...
        return html.Div([
            ...
            convert_mlb_df_to_html(df),
            html.Div(children=convert_mlb_stacks_to_html(app, stacks_df))
        ])
...
def stack_calculator_tab_content(tab, data, dropdown_selection):
    ...
    # If data is uploaded
        ...
        if dropdown_selection is None:
            ...
            df = summarize_mlb_lineup_stacks(df, points_ownership_df, PLAYER_TEAM_POS_DF, dropdown_selection)
