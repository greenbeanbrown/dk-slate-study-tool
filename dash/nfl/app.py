...
from functions import filter_dk_users, parse_uploaded_data, create_points_own_df, clean_entry_name, discrete_background_color_bins
from nfl_functions import *

#nfl_team_colors = {'ARI':'red','ATL':'blue','BAL':'orange','BOS':'red','CHC':'blue','CHW':'black','CIN':'red','CLE':'blue','COL':'purple','DET':'blue','HOU':'orange','KCR':'blue','LAA':'red','LAD':'blue','MIA':'orange','MIL':'blue','MIN':'blue','NYM':'orange','NYY':'blue','OAK':'green','PHI':'red','PIT':'yellow','SDP':'yellow','SFG':'orange','SEA':'black','STL':'red','TBR':'blue','TEX':'red','TOR':'blue','WAS':'red'}
...
def contest_summary_content(tab, data):
    ...
    if (data is None):
        ...
        # Create the points ownership df to pass through parse_nfl_lineup()
        ...
        return [
            ...
            convert_nfl_df_to_html(stack_frequency_df, style=None, page_size=10)]),
...
#            df = prep_raw_nfl_contest_data(df, 'MLB')[0][['player','nickname','position','points','ownership']]
...
#            df = filter_dk_users(prep_raw_nfl_contest_data(df, 'MLB')[1], prep_raw_nfl_contest_data(df, 'MLB')[0], dk_users)
#
...
#            convert_nfl_df_to_html(df, style='conditional')
#        ])
...
#        df = parse_nfl_lineup(df, points_ownership_df, PLAYER_TEAM_POS_DF, dk_user)
#
#        # Get stack data
#        stacks_df = calculate_nfl_stacks(df)
#        
#        return html.Div([
#            html.H3('Individual Lineup Analyzer'),
#            # Convert the processed data into an HTML table for output
#            convert_nfl_df_to_html(df),
#            html.Div(children=convert_nfl_stacks_to_html(app, stacks_df))
#        ])
...
#            df = summarize_nfl_lineup_stacks(df, points_ownership_df, PLAYER_TEAM_POS_DF)
#        else:
#            df = summarize_nfl_lineup_stacks(df, points_ownership_df, PLAYER_TEAM_POS_DF, dropdown_selection)
#
#        return html.Div([
